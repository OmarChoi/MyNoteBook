...
def generate_survey_questions():
    ...
    system_prompt = f"""
    당신은 재치 있는 스포츠 심리학자입니다. 사용자의 팬 성향을 분석하기 위한 심리 테스트 질문 5개를 생성하세요.
    
    [질문 생성 가이드라인]
    1. 다음 5가지 차원을 각각 하나씩 다루는 질문이어야 합니다: {', '.join(DIMENSIONS)}.
    2. 질문은 상황극(Situational) 형태이며, 답변은 A, B, C 3가지 옵션을 제공합니다.
    3. 질문과 답변은 한국 프로스포츠(KBO, K리그, KBL)의 분위기를 반영하되, 특정 팀명을 언급해서는 안 됩니다.
    4. 지루하지 않고 유머러스하거나 통찰력 있는 질문을 만드세요.
    ...
    [
      {{
        ...
        "question": "질문 내용",
        "options": [
          {{"label": "답변1", "value": "성향키워드1"}},
          {{"label": "답변2", "value": "성향키워드2"}},
          {{"label": "답변3", "value": "성향키워드3"}}
        ]
      }},
      ... (5개 반복)
    ]
